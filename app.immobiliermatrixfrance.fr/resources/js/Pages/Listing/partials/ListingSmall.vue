    ...
    <div
        ...
        <div class="flex-shrink-0 relative">
            ...
            <div class="absolute top-[25px] right-[25px]" style="z-index: 999">
                <button @click.prevent="toggleFavourite(listing.id)">
                    <HeartIcon v-if="!listing.user_favourited" class="w-8"></HeartIcon>
                    <HeartFilledIcon v-else class="w-8"/>
                </button>
            </div>
        </div>
        <div class="flex flex-1 flex-col justify-between bg-black p-6">
            <div class="flex-1">
                <div class="text-xl font-medium flex justify-between">
                    <div class="text-[#FF7C39] cursor-pointer"
                         @click="$inertia.visit(route('listing.show', listing.reference))">€ {{ listing.asking_price }}
                    </div>
                </div>
                <div class="mt-2 block cursor-pointer"
                     @click="$inertia.visit(route('listing.show', listing.reference))">
                    <p class="text-xl font-medium text-white">{{ listing.property_type }} {{ __('in').toLowerCase() }}
                        {{ listing.city }}</p>
...
const toggleFavourite = (listing) => {
    router.post(route('favourite-listing'), {listing: listing})
}
</script>
